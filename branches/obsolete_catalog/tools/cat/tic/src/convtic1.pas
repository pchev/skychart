...
{$R *.DFM}
type Ticrec = record gscr: array[1..4] of char;
                     s1  : char;
...
var
   ...
   ok : boolean;
begin
nom:=bpathi+'tic'+inttostr(ticnum);
...
id1:=strtoint(lin.gscr);
if (id1>=smmin) and (id1<=smmax) then begin
  inc(i);
  out.de := round((strtofloat(lin.de)/360000.0-90.0)*100000);
  out.ar := round((strtofloat(lin.ar)/360000)*100000);
  out.gscz:=id1;
  out.gscn:=strtoint(lin.gscn);
  out.mb := strtoint(lin.mb);
  if out.mb=0 then out.mb:=19900;
  out.mv := strtoint(lin.mv);
  if out.mv=0 then out.mv:=9900;
  fl16:=strtoint(lin.fl16);
  out.mb:=out.mb+(10000*fl16);
  write(fb,out);
