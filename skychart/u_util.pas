...
procedure SplitCmd(S : String; List : TStringList);
  ...
  begin
    ...
    if  (Length(Result) > 0) and
     (Result[1] = Result[Length(Result)]) and // if 1st char = last char and..
...
begin
  result :=
    padzeros(inttostr(a),4)+'-'+
    padzeros(inttostr(m),2)+'-'+
...
begin
  ...
  begin
    ...
    case year of
      ...
      result:=0; // we don't need deltat for very distant epoch as there is no available ephemeris
    end;
...
begin
  ...
  for i:=0 to obj.ControlCount-1 do
    if obj.Controls[i].visible then inc(result);
end;
