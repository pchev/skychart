...
uses
  Math, u_util, cu_smallsat;

type
  double20 = array[1..20] of double;

function MarSatAll(jde: double; var xsat, ysat, zsat: double20): integer;
function JupSatAll(jde: double; smallsat: boolean;
  var xsat, ysat, zsat: double20): integer;
function SatSatAll(jde: double; smallsat: boolean;
  var xsat, ysat, zsat: double20): integer;
function UraSatAll(jde: double; smallsat: boolean;
  var xsat, ysat, zsat: double20): integer;
function NepSatAll(jde: double; smallsat: boolean;
  var xsat, ysat, zsat: double20): integer;
...
type
  Tbody = (
    ...
    ABOVE_ORBIT, ALONG_PATH, BELOW_ORBIT, DEFAULT, MAJOR_PLANET, NAIF,
    NORAD, SAME_SYSTEM, UNKNOWN_BODY
    );
...
var
  F, E, F0, E0, eps, SF, CF, FF0, FPF0, SDIR, denom: double;
  i: integer;
begin
  if (L = 0) then
  begin
  ...
  begin
    ...
    if (denom = 1) and (E <= eps) and (FF0 <= eps) then
    begin
      Result := F;
      exit;
    end;
...
function jupsat(jd: double; b: Tbody; var X, Y, Z: double): boolean;
{
  ...
  procedure computeArguments(t: double;
  var l1, l2, l3, l4, om1, om2, om3, om4, psi, Gp, G: double);
  begin
    // mean longitudes
...
begin
  ...
  begin
    ...
    for i := 0 to ntr[ii, 4] - 1 do
      lon[ii] += series[ii][1][i][0] * sin(series[ii][1][i][1] +
        t * series[ii][1][i][2]);
  end;
end;

procedure SatcalcElem(jd: double; ii: integer; lon: array of double;
  var elem: array of double);
var
  t, s, phase, s1, s2: double;
  i, j: integer;
begin
  ...
  s := lon[ii] + al0[ii];
  for i := ntr[ii][4] to ntr[ii][1] - 1 do
  begin
...
var
  elem: array [0..5] of double = (0, 0, 0, 0, 0, 0);
...
var
  A, PHI, PSI, RKI, F, SF, CF, RLMF: double;
...
var
  alpha0, delta0, sa, sd, ca, cd, oldX, oldY, oldZ: double;
begin
...
var
  t, tcen, N1, N2, N3, N4, N5, E1, E2, E3, E4, E5: double;
...
begin
  ...
  case b of
    MIRANDA:
    begin
      N := (4443522.67 - 34.92 * cos(N1 - 3 * N2 + 2 * N3) +
        8.47 * cos(2 * N1 - 6 * N2 + 4 * N3) + 1.31 *
        cos(3 * N1 - 9 * N2 + 6 * N3) - 52.28 * cos(N1 - N2) - 136.65 *
        cos(2 * N1 - 2 * N2)) * 1e-6;

      L := (-238051.58 + 4445190.55 * t +
        25472.17 * sin(N1 - 3 * N2 + 2 * N3) - 3088.31 *
        sin(2 * N1 - 6 * N2 + 4 * N3) - 318.10 * sin(3 * N1 - 9 * N2 + 6 * N3) -
        37.49 * sin(4 * N1 - 12 * N2 + 8 * N3) - 57.85 *
        sin(N1 - N2) - 62.32 * sin(2 * N1 - 2 * N2) -
        27.95 * sin(3 * N1 - 3 * N2)) * 1e-6;

      K := (1312.38 * cos(E1) + 71.81 * cos(E2) +
        69.77 * cos(E3) + 6.75 * cos(E4) + 6.27 *
        cos(E5) - 123.31 * cos(-N1 + 2 * N2) + 39.52 *
        cos(-2 * N1 + 3 * N2) + 194.10 * cos(N1)) * 1e-6;

      H := (1312.38 * sin(E1) + 71.81 * sin(E2) +
        69.77 * sin(E3) + 6.75 * sin(E4) + 6.27 *
        sin(E5) - 123.31 * sin(-N1 + 2 * N2) + 39.52 *
        sin(-2 * N1 + 3 * N2) + 194.10 * sin(N1)) * 1e-6;

      Q := (37871.71 * cos(I1) + 27.01 * cos(I2) +
        30.76 * cos(I3) + 12.18 * cos(I4) + 5.37 * cos(I5)) * 1e-6;

      P := (37871.71 * sin(I1) + 27.01 * sin(I2) +
        30.76 * sin(I3) + 12.18 * sin(I4) + 5.37 * sin(I5)) * 1e-6;
    ...
    ARIEL:
    begin
      N := (2492542.57 + 2.55 * cos(N1 - 3 * N2 + 2 * N3) -
        42.16 * cos(N2 - N3) - 102.56 * cos(2 * N2 - 2 * N3)) * 1e-6;

      L := (3098046.41 + 2492952.52 * t -
        1860.50 * sin(N1 - 3 * N2 + 2 * N3) + 219.99 *
        sin(2 * N1 - 6 * N2 + 4 * N3) + 23.10 * sin(3 * N1 - 9 * N2 + 6 * N3) +
        4.30 * sin(4 * N1 - 12 * N2 + 8 * N3) - 90.11 *
        sin(N2 - N3) - 91.07 * sin(2 * (N2 - N3)) -
        42.75 * sin(3 * (N2 - N3)) - 16.49 * sin(2 * (N2 - N4))) * 1e-6;

      K := (-3.35 * cos(E1) + 1187.63 * cos(E2) +
        861.59 * cos(E3) + 71.50 * cos(E4) +
        55.59 * cos(E5) - 84.60 * cos(-N2 + 2 * N3) +
        91.81 * cos(-2 * N2 + 3 * N3) + 20.03 * cos(-N2 + 2 * N4) +
        89.77 * cos(N2)) * 1e-6;

      H := (-3.35 * sin(E1) + 1187.63 * sin(E2) +
        861.59 * sin(E3) + 71.50 * sin(E4) +
        55.59 * sin(E5) - 84.60 * sin(-N2 + 2 * N3) +
        91.81 * sin(-2 * N2 + 3 * N3) + 20.03 * sin(-N2 + 2 * N4) +
        89.77 * sin(N2)) * 1e-6;

      Q := (-121.75 * cos(I1) + 358.25 * cos(I2) +
        290.08 * cos(I3) + 97.78 * cos(I4) + 33.97 * cos(I5)) * 1e-6;

      P := (-121.75 * sin(I1) + 358.25 * sin(I2) +
        290.08 * sin(I3) + 97.78 * sin(I4) + 33.97 * sin(I5)) * 1e-6;
    ...
    UMBRIEL:
    begin
      N := (1515954.90 + 9.74 * cos(N3 - 2 * N4 + E3) -
        106.00 * cos(N2 - N3) + 54.16 * cos(2 * (N2 - N3)) -
        23.59 * cos(N3 - N4) - 70.70 * cos(2 * (N3 - N4)) -
        36.28 * cos(3 * (N3 - N4))) * 1e-6;

      L := (2285401.69 + 1516148.11 * t +
        660.57 * sin(N1 - 3 * N2 + 2 * N3) - 76.51 *
        sin(2 * N1 - 6 * N2 + 4 * N3) - 8.96 * sin(3 * N1 - 9 * N2 + 6 * N3) -
        2.53 * sin(4 * N1 - 12 * N2 + 8 * N3) - 52.91 *
        sin(N3 - 4 * N4 + 3 * N5) - 7.34 * sin(N3 - 2 * N4 + E5) -
        1.83 * sin(N3 - 2 * N4 + E4) + 147.91 * sin(N3 - 2 * N4 + E3) -
        7.77 * sin(N3 - 2 * N4 + E2) + 97.76 * sin(N2 - N3) +
        73.13 * sin(2 * (N2 - N3)) + 34.71 * sin(3 * (N2 - N3)) +
        18.89 * sin(4 * (N2 - N3)) - 67.89 * sin(N3 - N4) -
        82.86 * sin(2 * (N3 - N4)) - 33.81 * sin(3 * (N3 - N4)) -
        15.79 * sin(4 * (N3 - N4)) - 10.21 * sin(N3 - N5) -
        17.08 * sin(2 * (N3 - N5))) * 1e-6;

      K := (-0.21 * cos(E1) - 227.95 * cos(E2) +
        3904.69 * cos(E3) + 309.17 * cos(E4) +
        221.92 * cos(E5) + 29.34 * cos(N2) +
        26.20 * cos(N3) + 51.19 * cos(-N2 + 2 * N3) -
        103.86 * cos(-2 * N2 + 3 * N3) - 27.16 * cos(-3 * N2 + 4 * N3) -
        16.22 * cos(N4) + 549.23 * cos(-N3 + 2 * N4) +
        34.70 * cos(-2 * N3 + 3 * N4) + 12.81 * cos(-3 * N3 + 4 * N4) +
        21.81 * cos(-N3 + 2 * N5) + 46.25 * cos(N3)) * 1e-6;

      H := (-0.21 * sin(E1) - 227.95 * sin(E2) +
        3904.69 * sin(E3) + 309.17 * sin(E4) +
        221.92 * sin(E5) + 29.34 * sin(N2) +
        26.20 * sin(N3) + 51.19 * sin(-N2 + 2 * N3) -
        103.86 * sin(-2 * N2 + 3 * N3) - 27.16 * sin(-3 * N2 + 4 * N3) -
        16.22 * sin(N4) + 549.23 * sin(-N3 + 2 * N4) +
        34.70 * sin(-2 * N3 + 3 * N4) + 12.81 * sin(-3 * N3 + 4 * N4) +
        21.81 * sin(-N3 + 2 * N5) + 46.25 * sin(N3)) * 1e-6;

      Q := (-10.86 * cos(I1) - 81.51 * cos(I2) +
        1113.36 * cos(I3) + 350.14 * cos(I4) +
        106.50 * cos(I5)) * 1e-6;

      P := (-10.86 * sin(I1) - 81.51 * sin(I2) +
        1113.36 * sin(I3) + 350.14 * sin(I4) +
        106.50 * sin(I5)) * 1e-6;
    ...
    TITANIA:
    begin
      N := (721663.16 - 2.64 * cos(N3 - 2 * N4 + E3) -
        2.16 * cos(2 * N4 - 3 * N5 + E5) + 6.45 * cos(2 * N4 - 3 * N5 + E4) -
        1.11 * cos(2 * N4 - 3 * N5 + E3) - 62.23 * cos(N2 - N4) -
        56.13 * cos(N3 - N4) - 39.94 * cos(N4 - N5) -
        91.85 * cos(2 * (N4 - N5)) - 58.31 * cos(3 * (N4 - N5)) -
        38.60 * cos(4 * (N4 - N5)) - 26.18 * cos(5 * (N4 - N5)) -
        18.06 * cos(6 * (N4 - N5))) * 1e-6;

      L := (856358.79 + 721718.51 * t +
        20.61 * sin(N3 - 4 * N4 + 3 * N5) - 2.07 *
        sin(N3 - 2 * N4 + E5) - 2.88 * sin(N3 - 2 * N4 + E4) -
        40.79 * sin(N3 - 2 * N4 + E3) + 2.11 * sin(N3 - 2 * N4 + E2) -
        51.83 * sin(2 * N4 - 3 * N5 + E5) + 159.87 *
        sin(2 * N4 - 3 * N5 + E4) - 35.05 * sin(2 * N4 - 3 * N5 + E3) -
        1.56 * sin(3 * N4 - 4 * N5 + E5) + 40.54 * sin(N2 - N4) +
        46.17 * sin(N3 - N4) - 317.76 * sin(N4 - N5) -
        305.59 * sin(2 * (N4 - N5)) - 148.36 * sin(3 * (N4 - N5)) -
        82.92 * sin(4 * (N4 - N5)) - 49.98 * sin(5 * (N4 - N5)) -
        31.56 * sin(6 * (N4 - N5)) - 20.56 * sin(7 * (N4 - N5)) -
        13.69 * sin(8 * (N4 - N5))) * 1e-6;

      K := (-0.02 * cos(E1) - 1.29 * cos(E2) -
        324.51 * cos(E3) + 932.81 * cos(E4) +
        1120.89 * cos(E5) + 33.86 * cos(N2) +
        17.46 * cos(N4) + 16.58 * cos(-N2 + 2 * N4) +
        28.89 * cos(N3) - 35.86 * cos(-N3 + 2 * N4) -
        17.86 * cos(N4) - 32.10 * cos(N5) -
        177.83 * cos(-N4 + 2 * N5) + 793.43 * cos(-2 * N4 + 3 * N5) +
        99.48 * cos(-3 * N4 + 4 * N5) + 44.83 * cos(-4 * N4 + 5 * N5) +
        25.13 * cos(-5 * N4 + 6 * N5) + 15.43 * cos(-6 * N4 + 7 * N5)) * 1e-6;

      H := (-0.02 * sin(E1) - 1.29 * sin(E2) -
        324.51 * sin(E3) + 932.81 * sin(E4) +
        1120.89 * sin(E5) + 33.86 * sin(N2) +
        17.46 * sin(N4) + 16.58 * sin(-N2 + 2 * N4) +
        28.89 * sin(N3) - 35.86 * sin(-N3 + 2 * N4) -
        17.86 * sin(N4) - 32.10 * sin(N5) -
        177.83 * sin(-N4 + 2 * N5) + 793.43 * sin(-2 * N4 + 3 * N5) +
        99.48 * sin(-3 * N4 + 4 * N5) + 44.83 * sin(-4 * N4 + 5 * N5) +
        25.13 * sin(-5 * N4 + 6 * N5) + 15.43 * sin(-6 * N4 + 7 * N5)) * 1e-6;

      Q := (-1.43 * cos(I1) - 1.06 * cos(I2) -
        140.13 * cos(I3) + 685.72 * cos(I4) + 378.32 * cos(I5)) * 1e-6;

      P := (-1.43 * sin(I1) - 1.06 * sin(I2) -
        140.13 * sin(I3) + 685.72 * sin(I4) + 378.32 * sin(I5)) * 1e-6;
    ...
    OBERON:
    begin
      N := (466580.54 + 2.08 * cos(2 * N4 - 3 * N5 + E5) -
        6.22 * cos(2 * N4 - 3 * N5 + E4) + 1.07 * cos(2 * N4 - 3 * N5 + E3) -
        43.10 * cos(N2 - N5) - 38.94 * cos(N3 - N5) -
        80.11 * cos(N4 - N5) + 59.06 * cos(2 * (N4 - N5)) +
        37.49 * cos(3 * (N4 - N5)) + 24.82 * cos(4 * (N4 - N5)) +
        16.84 * cos(5 * (N4 - N5))) * 1e-6;

      L := (-915591.80 + 466692.12 * t -
        7.82 * sin(N3 - 4 * N4 + 3 * N5) + 51.29 *
        sin(2 * N4 - 3 * N5 + E5) - 158.24 * sin(2 * N4 - 3 * N5 + E4) +
        34.51 * sin(2 * N4 - 3 * N5 + E3) + 47.51 * sin(N2 - N5) +
        38.96 * sin(N3 - N5) + 359.73 * sin(N4 - N5) +
        282.78 * sin(2 * (N4 - N5)) + 138.60 * sin(3 * (N4 - N5)) +
        78.03 * sin(4 * (N4 - N5)) + 47.29 * sin(5 * (N4 - N5)) +
        30.00 * sin(6 * (N4 - N5)) + 19.62 * sin(7 * (N4 - N5)) +
        13.11 * sin(8 * (N4 - N5))) * 1e-6;

      K := (0 * cos(E1) - 0.35 * cos(E2) +
        74.53 * cos(E3) - 758.68 * cos(E4) +
        1397.34 * cos(E5) + 39.00 * cos(N2) +
        17.66 * cos(-N2 + 2 * N5) + 32.42 * cos(N3) +
        79.75 * cos(N4) + 75.66 * cos(N5) +
        134.04 * cos(-N4 + 2 * N5) - 987.26 * cos(-2 * N4 + 3 * N5) -
        126.09 * cos(-3 * N4 + 4 * N5) - 57.42 * cos(-4 * N4 + 5 * N5) -
        32.41 * cos(-5 * N4 + 6 * N5) - 19.99 * cos(-6 * N4 + 7 * N5) -
        12.94 * cos(-7 * N4 + 8 * N5)) * 1e-6;

      H := (0 * sin(E1) - 0.35 * sin(E2) +
        74.53 * sin(E3) - 758.68 * sin(E4) +
        1397.34 * sin(E5) + 39.00 * sin(N2) +
        17.66 * sin(-N2 + 2 * N5) + 32.42 * sin(N3) +
        79.75 * sin(N4) + 75.66 * sin(N5) +
        134.04 * sin(-N4 + 2 * N5) - 987.26 * sin(-2 * N4 + 3 * N5) -
        126.09 * sin(-3 * N4 + 4 * N5) - 57.42 * sin(-4 * N4 + 5 * N5) -
        32.41 * sin(-5 * N4 + 6 * N5) - 19.99 * sin(-6 * N4 + 7 * N5) -
        12.94 * sin(-7 * N4 + 8 * N5)) * 1e-6;

      Q := (-0.44 * cos(I1) - 0.31 * cos(I2) +
        36.89 * cos(I3) - 596.33 * cos(I4) + 451.69 * cos(I5)) * 1e-6;

      P := (-0.44 * sin(I1) - 0.31 * sin(I2) +
        36.89 * sin(I3) - 596.33 * sin(I4) + 451.69 * sin(I5)) * 1e-6;
...
begin
  ...
  for i := 1 to 2 do
  begin
    if marsat(jde, tbody(Ord(MARS) + i), X, Y, Z) then
    begin
      xsat[i] := X;
      ysat[i] := Y;
      zsat[i] := Z;
...
begin
  ...
  for i := 1 to 4 do
  begin
    if jupsat(jde, tbody(Ord(JUPITER) + i), X, Y, Z) then
    begin
      xsat[i] := X;
      ysat[i] := Y;
      zsat[i] := Z;
  ...
  if smallsat then
    for i := 1 to nrocks do
    begin
      if evaluate_rock(jde, rocks[i], vect) then
      begin
...
begin
  ...
  for i := 1 to 9 do
  begin
    if satsat(jde, tbody(Ord(SATURN) + i), X, Y, Z) then
    begin
      xsat[i] := X;
      ysat[i] := Y;
      zsat[i] := Z;
...
const
  nrocks = 13;
  rocks: array[1..nrocks] of integer =
...
begin
  ...
  for i := 1 to 5 do
  begin
    if urasat(jde, tbody(Ord(URANUS) + i), X, Y, Z) then
    begin
      xsat[i] := X;
      ysat[i] := Y;
      zsat[i] := Z;
...
begin
  ...
  for i := 1 to 2 do
  begin
    if nepsat(jde, tbody(Ord(NEPTUNE) + i), X, Y, Z) then
    begin
      xsat[i] := X;
      ysat[i] := Y;
      zsat[i] := Z;
...
begin
  ...
  for i := 1 to 1 do
  begin
    if plusat(jde, tbody(Ord(PLUTO) + i), X, Y, Z) then
    begin
      xsat[i] := X;
      ysat[i] := Y;
      zsat[i] := Z;
...
begin
  ...
  if isat <= 4 then
  begin
    if jupsat(jde, tbody(Ord(JUPITER) + isat), xs, ys, zs) then
    begin
      Result := 0;
    end
    else
  ...
  begin
    ...
    if isat <= nrocks then
    begin
      if evaluate_rock(jde, rocks[isat], vect) then
      begin
        xs := vect[0] / AU_to_km;
        ys := vect[1] / AU_to_km;
        zs := vect[2] / AU_to_km;
...
begin
  ...
  if isat <= 9 then
  begin
    if satsat(jde, tbody(Ord(SATURN) + isat), xs, ys, zs) then
    begin
      Result := 0;
    end
    else
...
begin
  ...
  if isat <= 5 then
  begin
    if urasat(jde, tbody(Ord(URANUS) + isat), xs, ys, zs) then
    begin
      Result := 0;
    end
    else
...
begin
  ...
  if isat <= 2 then
  begin
    if nepsat(jde, tbody(Ord(NEPTUNE) + isat), xs, ys, zs) then
    begin
      Result := 0;
    end
    else
...
begin
  ...
  if isat = 1 then
  begin
    if plusat(jde, tbody(Ord(PLUTO) + isat), xs, ys, zs) then
    begin
      Result := 0;
    end
    else
