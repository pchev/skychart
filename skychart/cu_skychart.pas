    ...
    begin
      ...
      begin
        ...
        Fplot.PlotDeepSkyObject(xx, yy, rec.neb.dim1, rec.neb.mag,
          ...
          rec.neb.morph, cfgsc.WhiteBg, True, rec.neb.color);
      end
      else
        Fplot.PlotDeepSkyObject(xx, yy, rec.neb.dim1, rec.neb.mag,
          rec.neb.sbr, abs(cfgsc.BxGlb) * deg2rad / rec.neb.nebunit, rec.neb.nebtype,
          rec.neb.morph, cfgsc.WhiteBg, False);
    end
    else
    begin
      ...
      begin
        ...
        Fplot.PlotDSOGxy(xx, yy, rec.neb.dim1, rec.neb.dim2, rec.neb.pa, 0,
          ...
          rec.neb.nebunit, rec.neb.morph, True, rec.neb.color);
      end
      else
        Fplot.PlotDSOGxy(xx, yy, rec.neb.dim1, rec.neb.dim2, rec.neb.pa, 0,
          100, 100, rec.neb.mag, rec.neb.sbr, abs(cfgsc.BxGlb) * deg2rad / rec.neb.nebunit,
          rec.neb.nebunit, rec.neb.morph, False, rec.neb.color);
    end;
  end;
...
begin
  ...
  if Fcatalog.OpenDSL(cfgsc.DSLforcecolor, cfgsc.DSLcolor) then
  begin
    fillchar(rec, sizeof(rec), 0);
    try
            ...
            begin
              ...
              begin
                ...
                if (cfgsc.fov > 0.1) and
                  ((abs(ps[0, j] - Fplot.cfgchart.hw) > 2 * Fplot.cfgchart.hw) or
                  ((abs(ps[1, j] - Fplot.cfgchart.hh) > 20 * Fplot.cfgchart.hh) and
                  (abs(cfgsc.hcentre) < 0.05)) or (abs(ps[0, j + hdiv + 1] - Fplot.cfgchart.hw) > 2 *
                  Fplot.cfgchart.hw) or ((abs(ps[1, j + hdiv + 1] - Fplot.cfgchart.hh) >
                  2 * Fplot.cfgchart.hh) and (abs(cfgsc.hcentre) < 0.05))) then
                begin
                  ok := False;
                  break;
                end;
