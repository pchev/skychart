...
begin
  ...
  bBigFont := (not OutlineActuallyVisible and (inherited TextSize('Hg').cy >= 24)) or
     (OutlineActuallyVisible and (inherited TextSize('Hg').cy > 42));
...
const DraftAntialiasingLevel = 2;
...
begin
  ...
  if Antialiasing then
    ...
    quality := fqSystem;
  size := BGRAOriginalTextSize(Font,quality,AText,DraftAntialiasingLevel);
  if (size.cx = 0) or (size.cy = 0) then
  begin
    size := BGRATextSize(Font,quality,'Hg',DraftAntialiasingLevel);
    FTextSize.cx := 0;
  ...
  if Antialiasing then
  begin
    sizeX := (sizeX + DraftAntialiasingLevel-1);
    sizeX -= sizeX mod DraftAntialiasingLevel;

    sizeY := (sizeY + DraftAntialiasingLevel-1);
    sizeY -= sizeY mod DraftAntialiasingLevel;

    if SubOffsetX <> 0 then
    begin
      sizeX += ceil(SubOffsetX*DraftAntialiasingLevel);
      iSubX := round(SubOffsetX*DraftAntialiasingLevel);
    end;
    if SubOffsetY <> 0 then
    begin
      sizeY += ceil(SubOffsetY*DraftAntialiasingLevel);
      iSubY := round(SubOffsetY*DraftAntialiasingLevel);
    end;

    OnePixel := DraftAntialiasingLevel;
  end else
  begin
    OnePixel := 1;
  ...
  if Antialiasing then
  begin
    FTextSize.cx := round(FTextSize.cx/DraftAntialiasingLevel);
    FTextSize.cy := round(FTextSize.cy/DraftAntialiasingLevel);
    FOffset := Point(round(FOffset.X/DraftAntialiasingLevel),round(FOffset.Y/DraftAntialiasingLevel));

    FTextMask := temp.Resample(round(temp.width/DraftAntialiasingLevel),round(temp.Height/DraftAntialiasingLevel),rmSimpleStretch);
