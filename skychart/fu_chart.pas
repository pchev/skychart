...
begin
 if printcolor<>2 then begin
   ...
   if prtsc.plot.cfgplot.plaplot=2 then prtsc.plot.cfgplot.plaplot:=1;
   ...
   if printcolor<2 then for i:=1 to numlabtype do prtsc.plot.cfgplot.LabelColor[i]:=clBlack;
   prtsc.plot.cfgplot.bgColor:=prtsc.plot.cfgplot.color[0];
 end;
end;
begin
if VerboseMsg then WriteTrace(caption+' PrintChart');
zoomstep:=0;
// create chart object for printing the first time we print
if prtImage=nil then prtImage:= TChartDrawingControl.Create(Self);
if prtsc=nil then begin
...
var desc,buf,buf2,otype,oname,txt,s1,s2,s3: string;
    ...
    r:TStringList;
    tp,ec,ap,an,ic,g,eq,cra,cdec,dist,dst,dkm,rr,elong,phase,magn,diam,lc,ctar,ctde,rc,xc,yc,zc,ma,sa,dx,dy,illum,vel : double;
...
ipla:=0;
isat:=0;
if (otype='P')or(otype='Ps')or(otype='S*') then begin
...
if otype='Cm' then begin
  if sc.cdb.GetComElem(sc.cfgsc.FindId,sc.cfgsc.TrackElemEpoch,tp,q,ec,ap,an,ic,h,g,eq,nam,elem_id) then begin
    ...
    if sc.cfgsc.PlanetParalaxe then Paralaxe(nst,dst,cra,cdec,cra,cdec,q,sc.cfgsc);
    if sc.cfgsc.ApparentPos then apparent_equatorial(cra,cdec,sc.cfgsc,true,false);
...
if (otype='S*') and (ipla=10) then begin
...
if (otype='Ps') and (ipla=11) then begin
...
if (otype='Ps') and (isat>0) then begin
...
if dist>(0.05/3600) then begin
  pa:=round(rmod(rad2deg*PositionAngle(sc.cfgsc.FindRA,sc.cfgsc.FindDec,cra,cdec)+360,360));
...
case n of
 ...
 68 : begin result:=cmd_SwitchGridNum; Refresh(true,false); end;
 69 : begin result:=cmd_SwitchConstL; Refresh(true,false); end;
 70 : begin result:=cmd_SwitchConstB; Refresh(true,false); end;
 71 : begin if sc.cfgsc.projpole<>equat then sc.cfgsc.projpole:=equat else sc.cfgsc.projpole:=altaz; refresh(true,false); end;
