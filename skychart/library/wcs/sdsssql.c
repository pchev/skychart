...
/* SDSS DR14 search engine former main server URL */
char sdssrurl[64]="http://skyserver.sdss.org/dr14/en/tools/search/X_Results.aspx";
...
int	nlog;		/* 1 for diagnostics */
{
    ...
    /* Set up SQL query for SDSS SQL server */
    ra = cra;
    dec = cdec;
    if (sysout != WCS_J2000)
    ...
    sprintf (srchurl, "?searchtool=SQL&TaskName=Skyserver.Search.SearchForm&format=csv");
    sprintf (temp, "select top %d p.objid, p.ra, p.dec, p.u, p.g, p.r, p.i, p.z from star p, ", nstar);
    strcat (srchurl, temp);
    ...
    sprintf (temp, "dbo.fgetNearByObjEq(%.5f,%.5f,%.3f) n", ra, dec, dtemp);
    strcat (srchurl, temp);
    sprintf (temp, "where p.objid=n.objid");
    strcat (srchurl, temp);
    ...
    if (mag1 < mag2) {
	sprintf (temp, " and (p.%c BETWEEN %.2f AND %.2f)", cmag, mag1, mag2);
	strcat (srchurl, temp);
	}
