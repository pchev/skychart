...
Function IsGSCCpath(path : shortstring) : Boolean; stdcall;
procedure SetGSCCpath(path : shortstring); stdcall;
Procedure OpenGSCC(ar1,ar2,de1,de2: double ; var ok : boolean); stdcall;
Procedure OpenGSCCwin(var ok : boolean); stdcall;
Procedure ReadGSCC(var lin : GSCCrec; var SMnum : shortstring ; var ok : boolean); stdcall;
...
Function IsGSCCpath(path : shortstring) : Boolean;
var p,buf : string;
...
procedure SetGSCCpath(path : shortstring);
begin
path:=noslash(path);
GSCCpath:=path;
end;
...
nomfich:=GSCCpath+slashchar+hemis+padzeros(nomzone,4)+slashchar+padzeros(nomreg,4)+'.gsc';
...
Procedure ReadGSCC(var lin : GSCCrec; var SMnum : shortstring ; var ok : boolean);
begin
