...
if [[ $make_linux ]]; then
  ...
  cd $builddir
  mkdir debian/skychart-data-stars/usr/
  mv share debian/skychart-data-stars/usr/
  cd debian
  sz=$(du -s skychart-data-stars/usr | cut -f1)
  sed -i "s/%size%/$sz/" skychart-data-stars/DEBIAN/control  
  ...
  mkdir rpm/SRPMS
  mkdir rpm/tmp
  mkdir -p rpm/skychart-data-stars/usr/
