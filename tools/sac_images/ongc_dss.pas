...
type
  ...
  private
    ...
    ngcH : TCatHeader;
    ngcinfo:TCatHdrInfo;
    ngcversion, MessierStrPos : integer;
   ...
   for i:=1 to 36 do begin
      ...
      if keyword='CONTRAS9' then
           try
             ...
             dmax:=(d1+d2) div 2;   // high value for good contrast
           except
             dmax:=dmaxi;
         end;
...
procedure TForm1.CheckMessierColumn;
var
  i: integer;
  trec: GCatrec;
begin
...
procedure TForm1.FormatGCatN(var rec: GcatRec);
var
  buf: string;
  i: integer;
begin
  ...
  begin
    ...
    begin
      // swap primary id
      buf := rec.neb.id;
      rec.neb.id := trim(rec.options.flabel[16]) + ' ' + trim(rec.str[MessierStrPos]);
...
if ok then
  repeat
    ...
    if maxvalue([rec.neb.dim1,rec.neb.dim2])>215 then begin
      writeln(log,'Skip too big:  '+nam);
      continue;
    end;

    if (rec.neb.nebtype<=0) or ((rec.neb.nebtype>6) and (rec.neb.nebtype<>13)) then begin
      writeln(log,'Skip type:'+inttostr(rec.neb.nebtype)+'  '+nam);
      continue;
    end;

    // use the following line for selective re-do.
    //if pos(nam+' ',' IC22 ')=0 then continue;
    ...
    cmd:='INSERT INTO cdc_fits (filename,catalogname,objectname,ra,de,width,height,rotation) VALUES ('
        ...
        +',"ONGC"'
        +',"'+nam+'"'
        +',"'+formatfloat(f4,deg2rad*rec.ra)+'"'
        +',"'+formatfloat(f4,deg2rad*rec.dec)+'"'
        +',"'+formatfloat(f5,0)+'"'
        +',"'+formatfloat(f5,0)+'"'
        +',"'+formatfloat(f5,0)+'"'+')';
    ...
    if (not db.query(cmd))and(copy(nam,1,1)<>'M') then begin
      ...
      img.SubSample:=sub;
      img.Ra:=deg2rad*rec.ra;
      img.De:=deg2rad*rec.dec;
      img.Width:=size;
      img.Height:=size;
      ...
      repeat
        ...
        if rc=0 then begin
          ...
          for n:=1 to pl.nplate do begin
             ...
             if (pl.dist_from_edge[n]>180) and
