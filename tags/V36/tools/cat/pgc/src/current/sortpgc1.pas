...
{$R *.lfm}
const
maxl = 300000;  // max objects per zone
...
var i :integer;
  lin : filbin;
begin
assignfile(fi,pathi+PathDelim+padzeros(inttostr(lgnum),2)+'.dat');
Reset(fi);
i:=0;
repeat
  inc(i);
...
var i,j :integer;
  lin : filbin;
begin
assignfile(fo,patho+PathDelim+padzeros(inttostr(lgnum),2)+'.dat');
Rewrite(fo);
