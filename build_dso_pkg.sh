...
if [[ $make_linux ]]; then
  ...
  cd $builddir
  mkdir debian/skychart-data-dso/usr/
  mv share debian/skychart-data-dso/usr/
  cd debian
  sz=$(du -s skychart-data-dso/usr | cut -f1)
  sed -i "s/%size%/$sz/" skychart-data-dso/DEBIAN/control  
  ...
  mkdir rpm/SRPMS
  mkdir rpm/tmp
  mkdir -p rpm/skychart-data-dso/usr/
