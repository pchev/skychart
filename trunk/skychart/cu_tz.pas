...
var
  ...
  gregorian: boolean;
begin
  if annee * 10000 + mois * 100 + jour >= FGregorianStart then
    gregorian := True
  else
    gregorian := False;
  u := annee;
...
begin
  u0 := jd + 0.5;
  if int(u0) >= FGregorianStartJD then
    gregorian := True
  else
    gregorian := False;
  u0 := jd + 32082.5;
...
begin
  ...
  if (timer = leaps[i].transition) and (((i = 0) and (leaps[i].change > 0)) or
    (leaps[i].change > leaps[i - 1].change)) then
  begin
    leap_hit := 1;
    while (i > 0) and (leaps[i].transition = leaps[i - 1].transition + 1) and
