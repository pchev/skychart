...
const
  ResampleFilterStr : array[TResampleFilter] of string =
   ('Box','Linear','HalfCosine','Cosine','Bicubic','Mitchell','Spline','BestQuality');
...
type
  TBGRAImageFormat = (ifUnknown, ifJpeg, ifPng, ifGif, ifBmp, ifIco, ifPcx, ifPaintDotNet, ifLazPaint, ifOpenRaster,
    ifPsd, ifTarga, ifTiff, ifXwd, ifXPixMap);
...
type
  ...
  public
     ...
     procedure TextOutAngle(x, y: single; orientation: integer; sUTF8: string; c: TBGRAPixel; align: TAlignment); virtual; abstract;
     procedure TextOutAngle(x, y: single; orientation: integer; sUTF8: string; texture: IBGRAScanner; align: TAlignment); virtual; abstract;
     ...
     procedure PutImageAffine(Origin,HAxis,VAxis: TPointF; Source: TBGRACustomBitmap; AOpacity: Byte=255); overload;
     procedure PutImageAffine(Origin,HAxis,VAxis: TPointF; Source: TBGRACustomBitmap; AOutputBounds: TRect; AOpacity: Byte=255); virtual; abstract; overload;
     function GetImageAffineBounds(Origin,HAxis,VAxis: TPointF; Source: TBGRACustomBitmap): TRect;
     procedure PutImageAngle(x,y: single; Source: TBGRACustomBitmap; angle: single; AOutputBounds: TRect; imageCenterX: single = 0; imageCenterY: single = 0; AOpacity: Byte=255; ARestoreOffsetAfterRotation: boolean = false); overload;
     procedure PutImageAngle(x,y: single; Source: TBGRACustomBitmap; angle: single; imageCenterX: single = 0; imageCenterY: single = 0; AOpacity: Byte=255; ARestoreOffsetAfterRotation: boolean = false); overload;
  ...
  TBGRACustomFontRenderer = class
    ...
    function GetFontPixelMetric: TFontPixelMetric; virtual; abstract;
    procedure TextOutAngle(ADest: TBGRACustomBitmap; x, y: single; orientation: integer; sUTF8: string; c: TBGRAPixel; align: TAlignment); virtual; abstract;
    procedure TextOutAngle(ADest: TBGRACustomBitmap; x, y: single; orientation: integer; sUTF8: string; texture: IBGRAScanner; align: TAlignment); virtual; abstract;
...
procedure TBGRACustomBitmap.PutImageAffine(Origin, HAxis, VAxis: TPointF;
  Source: TBGRACustomBitmap; AOpacity: Byte);
...
begin
  ...
  PutImageAffine(Origin,HAxis,VAxis,Source,outputBounds,AOpacity);
end;
...
begin
  ...
  PutImageAffine(Coord(0,0),Coord(source.Width,0),Coord(0,source.Height),source,AOutputBounds,AOpacity);
end;
...
var
  ...
  begin
    ...
    if (magic[0] = $0a) and (magic[1] in [0,2,3,4,5]) and (magic[2] = 1) and (magic[3] in[1,2,4,8]) then
      inc(scores[ifPcx],2);
